...
- `space` - tenant namespace (e.g. `cache`, `u-42`, `proj-archives`, `results-task-987`).
- `storage_key` - opaque object-store key (derived from `asset_id`); never exposed externally.
...
- **Remote URL timeout** - N/A (out of scope; remote fetching is delegated).
...
- **Partitioning approach** - by `space` for routing decisions; Postgres tables partitioned by `space` only if/when volume warrants (NFR-001 does not need it at 1 TB).
...
- **Cost controls** - per-space quotas; lifecycle rules on `pending` and `expired` prefixes; observability dashboards surface per-space storage usage so operators can flag runaway growth.
