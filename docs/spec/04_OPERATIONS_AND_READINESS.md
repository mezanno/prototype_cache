...
- **Release strategy** - rolling update per service; `asset-registry` and `storage-guard` are stateless and can be rolled one task at a time; `object-store` (MinIO) uses its own rolling strategy with quorum preserved; Postgres is updated during maintenance windows with read-only mode if required.
...
- **Secret manager** - Docker Swarm secrets for MVP (service credentials, Postgres passwords, MinIO root keys); migration to HashiCorp Vault or similar tracked as a forward step.
...
Plus standard MinIO and Postgres exporter metrics (request rate, latency, error rate, disk usage, connection pool).
...
- **Trace boundaries** - inbound HTTP request boundary; outbound MinIO and Postgres calls.
...
| ObjectStoreNodeDown | `up{job="minio"} == 0` for 5 min | SEV-1 | on-call |
| ObjectStoreDiskAlmostFull | MinIO node free space < 10% for 30 min | SEV-2 | on-call |
...
- [ ] Runbooks written for: capability issuance failures, MinIO node loss, Postgres failover, audit log overflow, garbage collection misfire.
- [ ] Dashboards and alerts live in the chosen observability backend; on-call ownership defined.
- [ ] Backup/restore tested for Postgres and MinIO (or chosen object-store).
...
- `RUNBOOK-001` MinIO node loss - detect, drain, replace, rebalance.
