...
class InMemoryAssetRegistryTest(unittest.TestCase):
    ...
    def test_reserves_commits_and_resolves_asset(self) -> None:
        ...
        self.assertEqual("gallica", pending.partition_id)
        self.assertEqual(
            "gallica/assets/" + pending.asset_id,
            pending.storage_key,
        )
        ...
        resolved = self.registry.resolve_alias(
            space="cache",
            alias="gallica/bnf/ark-12148/example/full/full/0/default.jpg",
        )
    ...
    def test_pending_and_expired_assets_do_not_resolve(self) -> None:
        ...
        with self.assertRaises(InvalidStateTransitionError):
            self.registry.resolve_alias(space="cache", alias="m/pending-image")
    ...
    def test_immutable_alias_rebind_is_rejected_by_default(self) -> None:
        ...
        with self.assertRaises(AliasImmutableError):
            self.registry.rebind_alias(
                space="users",
                alias="42/uploads/file.jpg",
                new_asset_id=second.asset_id,
                caller_service_id="admin",
            )

        resolved = self.registry.resolve_alias(space="users", alias="42/uploads/file.jpg")
    ...
    def test_mutable_alias_detach_then_rebind(self) -> None:
        ...
        self.registry.detach_mutable_alias(
            space="users",
            alias="42/mutable-pointer",
            caller_service_id="admin",
        )
        with self.assertRaises(InvalidStateTransitionError):
            self.registry.resolve_alias(space="users", alias="42/mutable-pointer")
        ...
        self.assertEqual(
            second.asset_id,
            self.registry.resolve_alias(space="users", alias="42/mutable-pointer").asset_id,
        )
        rebind_events = [
    ...
    def test_detach_immutable_sets_tombstone(self) -> None:
        ...
        reg.detach_alias(space="cache", alias="m/gone", caller_service_id="svc")
    ...
    def test_detach_routes_mutable_vs_immutable(self) -> None:
        ...
        with self.assertRaises(AliasImmutableError):
            reg.detach_alias(space="cache", alias="p/m", caller_service_id="svc")
        ...
        with self.assertRaises(AliasImmutableError):
            reg.detach_mutable_alias(space="cache", alias="p/i", caller_service_id="svc")
    ...
    def test_delete_asset_blocks_resolve(self) -> None:
        ...
        with self.assertRaises(InvalidStateTransitionError):
            self.registry.resolve_alias(space="cache", alias="m/z")
